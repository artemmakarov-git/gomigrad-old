...
function scout.Think()
    scout.LastWave = scout.LastWave or CurTime() + 1

    if CurTime() >= scout.LastWave then
        SetGlobalInt("scout_respawntime", CurTime())
 ...
 --[[       for ent in pairs(scout.ragdolls) do
            if IsValid(ent) then ent:Remove() end
            scout.ragdolls[ent] = nil
        end]]
        scout.LastWave = CurTime() + 5
    end
end

function scout.SpawnCommand(tbl,aviable,func,funcShould)
...
function scout.DirectOtherTeam(start,min,max)
...
function scout.GetListMul(list,mul,func,max)
	local newList = {}
...
function scout.StartRoundSV()
	...
	local spawnsT,spawnsCT = scout.SpawnsTwoCommand()
	scout.SpawnCommand(team.GetPlayers(1),spawnsT)
	scout.SpawnCommand(team.GetPlayers(2),spawnsCT)
...
function scout.GetCountLive(list,func)
	local count = 0
	local result
...
function scout.RoundEndCheck()
--	scout.Center()
	if roundTimeStart + roundTime - CurTime() <= 0 then EndRound() end
	local TAlive = scout.GetCountLive(team.GetPlayers(1))
	local CTAlive = scout.GetCountLive(team.GetPlayers(2))
...
function scout.EndRoundMessage(winner,textNobody)
	local tbl = TableRound()
	PrintMessage(3,"Выиграли - " .. ((winner == 1 and tbl.red[1]) or (winner == 2 and tbl.blue[1]) or (textNobody or "Дружба")) .. ".")
end

function scout.EndRound(winner)
	net.Start("nomuzzle")
    net.Broadcast()
	RunConsoleCommand("sv_gravity", "600")
	scout.EndRoundMessage(winner)
end

--

function scout.GiveSwep(ply,list,mulClip1)
...
function scout.PlayerSpawn(ply,teamID)
	local teamTbl = scout[scout.teamEncoder[teamID]]
	...
	scout.GiveSwep(ply,teamTbl.main_weapon)
	scout.GiveSwep(ply,teamTbl.secondary_weapon)
...
function scout.PlayerInitialSpawn(ply) ply:SetTeam(math.random(2)) end

function scout.PlayerCanJoinTeam(ply,teamID)
    if teamID == 3 then ply:ChatPrint("Иди нахуй") return false end
end

function scout.PlayerDeath(ply,inf,att) return false end