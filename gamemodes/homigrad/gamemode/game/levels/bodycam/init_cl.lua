...
function bodycam.StartRoundCL()
    playsound = true
end

function bodycam.HUDPaint_RoundLeft(white)
    local lply = LocalPlayer()
	local name,color = bodycam.GetTeamName(lply)
...
hook.Add("player_activate","bodycam_SendData",function(data)
    bodycam.points = {}

    bodycam.WinPoints = {}

    for i = 1, 2 do
        bodycam.WinPoints[i] = GetGlobalInt("bodycam_Winpoints" .. i)
    end

    timer.Create("bodycam_ThinkAboutPoints", 1, 0, function() --подумай о точках... засунул в таймер для оптимизации, ибо там каждый тик игроки в сфере подглядываются, ну и в целом для удобства
        bodycam.PointsThink()
    end)

    for k, v in pairs(bodycam.points) do
...
function bodycam.PointsThink()
    for i = 1, 3 do
        bodycam.WinPoints[i] = GetGlobalInt("bodycam_Winpoints" .. i, 0)
    end
...
function bodycam.HUDPaint_RoundLeft(white2) --позиции точек и счёт
    local lply = LocalPlayer()
	local name,color = bodycam.GetTeamName(lply)
    ...
    surface.DrawRect(ScrW() * 0.39 + ((1000 - bodycam.WinPoints[1]) / 1000 * ScrW() * 0.1), ScrH() * 0.97, bodycam.WinPoints[1] / 1000 * ScrW() * 0.1, ScrH() * 0.01)
    --print(bodycam.WinPoints[1])
    --print("gega")
    --print(bodycam.WinPoints[2])
    surface.SetDrawColor(blue)
    surface.DrawRect(ScrW() * 0.51, ScrH() * 0.97, bodycam.WinPoints[2] / 1000 * ScrW() * 0.1, ScrH() * 0.01)
    ...
    local respawntime = GetGlobalInt("bodycam_respawntime", CurTime())
