starwarsclonewars.GetTeamName = tdm.GetTeamName
...
hook.Add("player_activate","starwarsclonewars_SendData",function(data)
    cp.points = {}

    cp.WinPoints = {}

    for i = 1, 2 do
        cp.WinPoints[i] = GetGlobalInt("starwarsclonewars_Winpoints" .. i)
    end
...
function starwarsclonewars.PointsThink()
    for i = 1, 3 do
        cp.WinPoints[i] = GetGlobalInt("starwarsclonewars_Winpoints" .. i, 0)
    end
...
function starwarsclonewars.StartRoundCL()
    playsound = true
end

function starwarsclonewars.HUDPaint_RoundLeft(white)
    local lply = LocalPlayer()
	local name,color = starwarsclonewars.GetTeamName(lply)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "Войны клонов", "HomigradFontBig", ScrW() / 2, ScrH() / 8, Color( 255,255,255,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
    ...
    local respawntime = GetGlobalInt("starwarsclonewars_respawntime", CurTime())
