...
function deathrun.HUDPaint_RoundLeft(white2,time)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        draw.DrawText( "убегающие", "HomigradFontBig", ScrW() / 2, ScrH() / 8, Color( 155,55,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        ...
        if lply:Team() == 1 then
            ...
            draw.DrawText( "Вам нужно пройти испытание Смерти, и выжить.", "HomigradFontBig", ScrW() / 2, ScrH() / 1.2, Color( 55,155,55,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
		end
        return
    end
	...
	*/
	...
	if lply:Team() == 3 or not lply:Alive() and deathrun.police then
